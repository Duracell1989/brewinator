...
- `VersionedNotesPage` holds the preamble the three "one page per exact version" sources repeated verbatim - fill a `%s` template, fetch, and turn a transport error, a bad status or an undecodable body into the right `Result`. Firefox, NSS and VLC each carried their own copy, down to the shared "invalid release notes URL" string, so the 404-policy fix above would otherwise have had to be made three times or left to diverge. Only the preamble moved: the three sources genuinely differ after it, and folding the whole `fetch` in would have meant a parameter per difference. A 404 is now an explicit `NotFoundPolicy` at each call site rather than an implicit consequence of how the guard was written.
- The plain tag strip lives on `HTMLTextReducer` as `removingTags`, instead of once per vendor source. `reduce`'s own version had already drifted to `<[^>]+>`, which is the divergence this prevents.
